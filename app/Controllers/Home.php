...
class Home extends BaseController
{
        ...
        if ($order == 'laporan') {
            ...
            if ($jenis == "Backup") {
                foreach (tahuns('transaksi') as $t) {
                    ...
                    $q = db('backup')->where('tahun', $t['tahun'])->where('lokasi', $lokasi)->get()->getRowArray();
                    ...
                    $backup = db('backup')->select('*')->where('lokasi', $lokasi)->orderBy('tahun', 'ASC')
                        ->get()
                        ->getResultArray();
    ...
    public function unlock()
    {
        ...
        if (db('backup')->where('id', $q['id'])->update($q)) {
            $backup = db('backup')->select('*')->where('lokasi', user()['lokasi'])->orderBy('tahun', 'ASC')
                ->get()
                ->getResultArray();
            $tot = array_sum(array_column($backup, 'saldo'));
